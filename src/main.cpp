...
/**
 * App configuration setup for this program. Decides what to do with the user's input and which features to run.
 */
struct AppConfig {
...
/**
 * Sets the application configuration to set up and initiate the ADASFeature implementations along with other options
 * that the user selected.
 * @param argc Arguments count
 * @param argv Argument list for our program.Eg: --log --show=... --store=... --video=..., etc
 * @return An optional AppConfig if all arguments are valid and no errors occurred. If video is not entered then it
 * returns noopt
 */
std::optional<AppConfig> parse_arguments(int argc, char **argv);
/**
 * Puts FPS performance on the image.
 * @param src The Frame to modified and put FPS
 * @param start_time The starting time our Feature implementations received the image.
 * @param end_time The ending time our Feature implementations finished analyzing and annotating the image.
 */
void put_fps_text(cv::Mat &src, const steady_clock::time_point &start_time, const steady_clock::time_point &end_time);
/**
 * Runs all user selected features to apply analysis and annotations. All annotated frames from each feature will
 * then be combined to form one annotated frame.
 * @param frame The frame that all user selected features will analyze and annotate.
 * @param adas_features The ADAS Feature implementations the user selected to perform analysis and annotation.
 * @param pool The thread pool that all features will run on while waiting for jobs to avoid blocking code and keep
 * user response.
 * @return The combined annotated features from all user selected implemented features.
 */
