...
YOLOVideoDetector::YOLOVideoDetector(const string &modelPath, const string &classFilePath,
...
void YOLOVideoDetector::loadModel(const std::string &modelPath) {
  ...
  try {
    ...
    std::cerr << "Warning: Failed to initialize CUDA provider (" << e.what() << "). Falling back to CPU execution.\n";
  }
  /********** CUDA MODIFIED **************/
#else
...
cv::Mat YOLOVideoDetector::process(cv::Mat &frame) {
  ...
  for (int idx : indices) {
    ...
    logDetection(frameNumber,      // Num of frame
                 className,        // Class
...
std::string YOLOVideoDetector::get_feature_name() { return "objects"; }
                                      ...
                                      vector<Rect> &boxes)        // List of boxes created by YOLO model
{
  ...
  std::vector<float> inputTensorValues(3 * INPUT_WIDTH * INPUT_HEIGHT);
  ...
  for (int y = 0; y < INPUT_HEIGHT; y++) {
    for (int x = 0; x < INPUT_WIDTH; x++) {
      ...
      // Red channel
      inputTensorValues[0 * channelSize + y * INPUT_WIDTH + x] = pixel[0] / 255.0f;

      // Green channel
      inputTensorValues[1 * channelSize + y * INPUT_WIDTH + x] = pixel[1] / 255.0f;

      // Blue channel
      inputTensorValues[2 * channelSize + y * INPUT_WIDTH + x] = pixel[2] / 255.0f;
    }
  }

  std::array<int64_t, 4> inputShape = {1, 3, INPUT_HEIGHT, INPUT_WIDTH};
  ...
  int dims[] = {(int)shape[0], (int)shape[1], (int)shape[2]};

  Mat output(3, dims, CV_32F, outputData);
  ...
  for (int i = 0; i < output.rows; i++) {
    ...
    boxes.emplace_back(
        Rect(static_cast<int>(left), static_cast<int>(top), static_cast<int>(width), static_cast<int>(height)));
                                      ...
                                      float confidence)    // Confidence
{
  ...
  Size textSize = getTextSize(displayLabel, FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseline);

  // Calculate the location of the box
  int y = max(box.y, textSize.height);

  // draw the box in the frame
  rectangle(frame, Point(box.x, y - textSize.height - 4), Point(box.x + textSize.width, y + baseline),
            Scalar(0, 255, 0), FILLED);

  // Write the label at the top left corner of the box
  putText(frame, displayLabel, Point(box.x, y - 2), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 0), 1);
}
                                ...
                                int64 &previousTick) // previous timestamp to calculate speed
{
  ...
  double elapsed = static_cast<double>(currentTick - previousTick) / getTickFrequency();
  ...
  // Formating
  Size fpsSize = getTextSize(fpsText, FONT_HERSHEY_SIMPLEX, 0.7, 2, &baseline);
  ...
  rectangle(frame, Point(fpsOrigin.x - 5, fpsOrigin.y - fpsSize.height - 5),
            Point(fpsOrigin.x + fpsSize.width + 5, fpsOrigin.y + baseline + 5), Scalar(0, 0, 0), FILLED);

  // Write the processing speed over the background using white color
  putText(frame, fpsText, fpsOrigin, FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255, 255, 255), 2);
}
...
string YOLOVideoDetector::getClassName(int classId) const {
  if (classId >= 0 && static_cast<size_t>(classId) < classNames.size()) {
...
void YOLOVideoDetector::logDetection(int frameNumber, const std::string &className, float confidence,
                                     const cv::Rect &box, const cv::Size &frameSize) {
  if (!logFile.is_open())
    return;
  ...
  logFile << frameNumber << "," << className << "," << std::fixed << std::setprecision(3) << confidence << ","
